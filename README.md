...
```
oscap-lxd instancename xccdf eval INPUT_CONTENT
...
The following command evaluates a Fedora instance as root. HTML report is written out as report.html on the local machine. Can be executed from any machine that has lxc and bash. The local machine does not need openscap installed.

```
./oscap-lxd my-fedora xccdf eval --profile xccdf_org.ssgproject.content_profile_common --report report.html /usr/share/xml/scap/ssg/content/ssg-fedora-ds.xml
```

The output:
```
Copying input file '/usr/share/xml/scap/ssg/content/ssg-fedora-ds.xml' to instance working directory '/tmp/tmp.yEsdWV54ry'...
...
Removing instance temporary directory...
```

## Example 2

A more full example, uses a tailoring file and also copies back ARF, XCCDF results. The tailoring file is copied from local machine to instance.

```
./oscap-ssh my-fedora xccdf eval --profile xccdf_org.ssgproject.content_profile_common --report report.html --results results.xml --results-arf arf.xml --tailoring-file ssg-fedora-ds-tailoring.xml /usr/share/xml/scap/ssg/content/ssg-fedora-ds.xml
```

The output:
```
Copying input file '/usr/share/xml/scap/ssg/content/ssg-fedora-ds.xml' to instance working directory '/tmp/tmp.yVy6snyC88'...
ssg-fedora-ds.xml                                                                                                                                                                                            100%  683KB 683.2KB/s   00:00    
Copying tailoring file 'ssg-fedora-ds-tailoring.xml' to instance working directory '/tmp/tmp.yVy6snyC88'...
...
## [oscap-ssh](https://github.com/mpreisler/oscap-ssh) port for lxd

This script does exactly the same as `oscap-ssh` did:

- Instead of `ssh` to a machine `lxc exec` is called to an instance
- Instead of `scp` to a machine `lxc file push/pull` is called to an instance
